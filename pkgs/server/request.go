...
func GetLimit(r *http.Request, d int) (int, error) {
	asStr := GetParam(r, "limit", "")
	if asStr == "" {
		return d, nil
	}
...
// GetLimit is a shotcut to get the limit from the request URL parameters
func GetOffset(r *http.Request, d int) (int, error) {
	asStr := GetParam(r, "offset", "")
	if asStr == "" {
		return d, nil
	}
